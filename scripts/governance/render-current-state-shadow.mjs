...
export function validateStateShape(state) {
  ...
  if (!action || typeof action !== 'object') errors.push('next_authorizable_action must be an object');
    ...
    if (action.order_id !== 'GOVERNANCE-EFFICIENCY-REFOUNDATION-LEDGER-PARTITION-SHADOW-HARDENING-R2') errors.push('next_authorizable_action.order_id mismatch');
    if (action.canonical_value !== 'DIRECT SUPERVISOR REVIEW OF GOVERNANCE-EFFICIENCY-REFOUNDATION-LEDGER-PARTITION-SHADOW-HARDENING-R2') errors.push('next_authorizable_action.canonical_value mismatch');
