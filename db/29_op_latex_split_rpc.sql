-- =====================================================================
-- db/29_op_latex_split_rpc.sql
-- RAVATEX-TAPETES-OP-PARTIAL-SPLIT-DB29-RPC-B
--
-- 1. Atualiza gerar_op_latex para filtrar apenas OPs default
--    (motivo_separacao IS NULL) no SELECT e no ON CONFLICT,
--    conforme exigido pelo novo indice parcial da db/28.
--
-- 2. Cria gerar_op_latex_split(p_entrega_id, p_motivo) para split
--    excepcional explicito com OP propria, motivo registrado,
--    idempotencia por entrega e rastro em op_eventos.
--
-- Depende de db/28 (ops.motivo_separacao + indices parciais).
...
COMMENT ON FUNCTION public.gerar_op_latex(BIGINT) IS
  'db/29: Cria ou reutiliza OP de Acabamento/Latex consolidada por (origem_op_id, destino_fornecedor_id) apenas para OPs default (motivo_separacao IS NULL). Retorna flags created/accumulated/already_linked.';
...
BEGIN
  ...
  -- Resolver lote da OP origem
  SELECT lote_id INTO v_lote_id FROM public.ops WHERE id = v_op_id;
  IF v_lote_id IS NULL THEN
    RAISE EXCEPTION 'OP origem % sem lote_id — split requer lote valido', v_op_id;
  END IF;
...
COMMENT ON FUNCTION public.gerar_op_latex_split(BIGINT, TEXT) IS
  'db/29: Cria OP Latex excepcional com motivo_separacao = p_motivo. Idempotente por entrega (op_latex_entregas). Registra rastro em op_eventos (criacao_split + split_derivado). Admin-only.';
