...
// remote database:
//   - the ordered deployment manifest resolves exactly db/01..db/78, with
//     db/77 and db/78 as the terminal two, fixed migration numbers unique
...
function buildDeploymentManifest({ dbDir = DB_DIR, applicationArtifact = APPLICATION_ARTIFACT, expectedBranch = EXPECTED_BRANCH } = {}) {
  ...
  assert.equal(terminalTwo[0].filename, DB77_FILENAME);
  assert.equal(terminalTwo[1].filename, DB78_FILENAME);
...
test('deployment manifest resolves exactly db/01..db/78, contiguous and unique', () => {
...
test('db/77 and db/78 are the terminal two migrations', () => {
  const filenames = fs.readdirSync(DB_DIR);
  const entries = resolveMigrationManifest(filenames, { expectedTerminal: EXPECTED_TERMINAL });
  const [second77, first78] = entries.slice(-2);
  assert.equal(second77.filename, DB77_FILENAME);
  assert.equal(first78.filename, DB78_FILENAME);
});

test('the full deployment manifest builds against the real repository', () => {
  const manifest = buildDeploymentManifest();
  assert.equal(manifest.migrations.length, 78);
