...
// remote database:
//   - the ordered deployment manifest resolves exactly db/01..db/79, with
//     db/78 and db/79 as the terminal two, fixed migration numbers unique
...
// artifact, ordered db/01..db/79 sequence, terminal two migrations with
...
function buildDeploymentManifest({ dbDir = DB_DIR, applicationArtifact = APPLICATION_ARTIFACT, expectedBranch = EXPECTED_BRANCH } = {}) {
  ...
  assert.equal(terminalTwo[0].filename, DB78_FILENAME);
  assert.equal(terminalTwo[1].filename, DB79_FILENAME);
...
test('deployment manifest resolves exactly db/01..db/79, contiguous and unique', () => {
...
test('db/78 and db/79 are the terminal two migrations', () => {
  const filenames = fs.readdirSync(DB_DIR);
  const entries = resolveMigrationManifest(filenames, { expectedTerminal: EXPECTED_TERMINAL });
  const [second78, first79] = entries.slice(-2);
  assert.equal(second78.filename, DB78_FILENAME);
  assert.equal(first79.filename, DB79_FILENAME);
});

test('the full deployment manifest builds against the real repository', () => {
  const manifest = buildDeploymentManifest();
  assert.equal(manifest.migrations.length, 79);
