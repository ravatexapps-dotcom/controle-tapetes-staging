...
> **Higiene documental:** runners operacionais
> (`RAVATEX-TAPETES-AUTH-DISABLE-USER-UI-BROWSER-E2E-A` e
> similares) não pertencem a este ledger — são ferramentas
> operacionais, não refactor/modularização. Passagens
> operacionais longas foram removidas; validação manual de UI é
> registrada em `PROJECT_STATE.md` e `AGENT_HANDOFF.md`.
...
## 5l. Higiene documental — runners operacionais fora do escopo do refactor

Runners operacionais como
`RAVATEX-TAPETES-AUTH-DISABLE-USER-E2E-AUTO-RUNNER-A` e
`RAVATEX-TAPETES-AUTH-DISABLE-USER-UI-BROWSER-E2E-A` são
ferramentas de validação, **não** fases de refactor/modularização.
Por isso seus detalhes operacionais (comandos, pré-requisitos,
fluxos passo-a-passo, garantias) **não** pertencem a este
ledger. A validação operacional (manual ou automatizada) é
registrada em `PROJECT_STATE.md` e `AGENT_HANDOFF.md`. Este
ledger mantém apenas histórico de decisões arquiteturais
reais.
...
integração da tela `#/cadastros/usuarios` com `admin-disable-user`)
estão **concluídos (local)**. Teste fornecedor 403 confirmado em
...
`result: PASS`** (ver §5k). **Validação manual da UI em staging**
(operacional) registrada em `PROJECT_STATE.md` e
`AGENT_HANDOFF.md` — não pertence a este ledger. **Não
avançar** para produção `bhgifjrfagkzubpyqpew` sem autorização
explícita.
