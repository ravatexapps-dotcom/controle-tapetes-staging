...
NEXT_AUTHORIZABLE_ACTION: supervisor review and acceptance/closeout of the IMPLEMENTED / LOCALLY VERIFIED / AWAITING SUPERVISOR REVIEW PHASE-C5A-DB-EMISSION-READINESS implementation (db/77_ordem_compra_c5a_emission_readiness.sql — grant EXECUTE on emitir_ordem_compra(BIGINT) to authenticated with the is_admin() gate authoritative + read-model correction of obter_ordem_compra_admin/listar_ordens_compra_admin; tests/ordem-compra-c5a-emission-readiness.integration.sql; contract §23), including review of the forced migration-manifest fixture update to tests/ordem-compra-c3d-deploy.smoke.js (frozen PHASE-C3D-A guard advanced terminal 76→77, one file beyond the order's literal Part 2 manifest, non-weakening), then a separately authorized non-production apply of db/77 with the contract §14 shared-environment evidence; PHASE-C5 UI implementation, PHASE-C5B-ACCEPTANCE-DECISION (IDENTIFIED / NOT AUTHORIZED), any shared-database apply of db/77, staging validation/application of db/76 or db/77, and the REAL_CUTOVER window (OC-CUTOVER-001/OC-CUTOVER-PONR-001, hard-gated behind the mandatory separate read-only completeness disposition of the 13 unmapped ordens_compra_fio rows ids 153–165) remain unauthorized
...
| OC-C5-EMISSION-001 | docs/architecture/ORDEM_COMPRA_LIFECYCLE_SPEC_PROPOSED.md::§R.24.10 | C5 | BLOCKED | — | — | NOT_ACTIVATED | — | Disposition label per the supervisor's exact ruling: PLANNED / BLOCKED_BY_C5A_DB_PREREQUISITE (recorded here as the closest single-token allowed value BLOCKED, per this matrix's own ALLOWED_DISPOSITIONS enum enforced by scripts/spec-custody/validation-core.mjs — the compound string itself is not a valid DISPOSITION-column value; the exact compound label is preserved verbatim in PROJECT_STATE.md, AGENT_HANDOFF.md, docs/ledgers/G28_LEDGER.md, and contract §21). Native emission remains a separate post-C4 gate. docs/architecture/ORDEM_COMPRA_C5_PHASE_CONTRACT.md is ACCEPTED / IMPLEMENTATION BLOCKED BY DATABASE PREREQUISITE (§21, supervisor-accepted under C5-CONTRACT-ACCEPTANCE-CLOSEOUT-R1, accepted contract commit f9fa97703d2724d62a0d916cca7b9637d54a1e08) — binds this requirement to an exact functional scope, state/action matrix, and purely-additive three-file manifest; database-prerequisite classification BLOCKING_DATABASE_PREREQUISITE ratified, assigned to a new separately authorized PHASE-C5A-DB-EMISSION-READINESS (not yet authored); the missing status_aceite pendente-to-aceita/rejeitada transition RPC gap ratified as PHASE-C5B-ACCEPTANCE-DECISION (IDENTIFIED / NOT AUTHORIZED); emission confirmation UX ratified CONTROLLED_IRREVERSIBLE_TRANSITION. Not SATISFIED — implementation not authorized; contract acceptance does not activate the requirement. The database prerequisite is specified to migration-ready precision by docs/architecture/ORDEM_COMPRA_C5A_DB_EMISSION_READINESS_PHASE_CONTRACT.md, which the supervisor ACCEPTED under C5A-DB-EMISSION-READINESS-IMPLEMENTATION-R1 (Part 1, contract §22; STATUS: ACCEPTED / IMPLEMENTATION AUTHORIZED LOCALLY; authored C5A-DB-EMISSION-READINESS-CONTRACT-R1): classification READ_MODEL_FUNCTION_AND_GRANT_PREREQUISITE — one migration (db/77) grants EXECUTE on emitir_ordem_compra(BIGINT) to authenticated (terminally REVOKE ALL, db/74:1192-1193) and corrects the terminal read models obter_ordem_compra_admin (db/69:987) / listar_ordens_compra_admin (db/69:913), which hard-code pode_emitir/acoes.emitir=false; allocation is ALLOCATION_PATH_READY_AFTER_GRANT via the already-granted definir_alocacao_necessidade_compra_fio (db/74:1177), so the superseded alocar_necessidade_compra_fio needs no grant; acceptance disposition EMISSION_ALLOWED_ONLY_WHEN_EXIGE_ACEITE_FALSE. PHASE-C5A-DB-EMISSION-READINESS is now ACTIVE; its db/77 grant + read-model correction is IMPLEMENTED / LOCALLY VERIFIED / AWAITING SUPERVISOR REVIEW (contract §23, verified on a disposable local PostgreSQL 18.4 cluster with ordered db/01..db/77 + the 64-row corpus 64/51/51/51/51, clean apply + idempotent reapply, tests/ordem-compra-c5a-emission-readiness.integration.sql PASS; the authorized new migration forced a minimal, non-weakening migration-manifest fixture update to tests/ordem-compra-c3d-deploy.smoke.js terminal 76->77). Under C5A-DB77-SHARED-DEV-VALIDATION-R1 (2026-07-22) db/77 was then applied and validated in the shared development database ucrjtfswnfdlxwtmxnoo (authorized non-production apply; PostgreSQL 17.6; terminal migration 20260722055832; byte-identical to e7a8b761, SHA-256 9628a947...; clean apply + idempotent reapply; emitir body byte-unchanged md5 495692f2..., grant-only; read models corrected to derive readiness; exact grant matrix emitir->authenticated only; C3C guard untouched md5 00ea8e98...; full contract §14/§24 behavioral evidence — allocation writer, authorized authenticated emission, non-admin/anon denials, wrong-state/incomplete/missing-supplier/zero-item denials, over-allocation, duplicate allocation/emission idempotency, one audit event, acceptance init nao_aplicavel with no fabricated decision, detail+list read-model readiness, exige_aceite gate, inert states, atomic invariance, and the cutover fence permit/deny proved non-persistently; zero validation residue; cutover unchanged legacy_active) — now SHARED-DEVELOPMENT VERIFIED / AWAITING SUPERVISOR CLOSEOUT. No staging, no deployment, no REAL_CUTOVER, no production access; no push. This does not activate OC-C5-EMISSION-001 — the emission UI (PHASE-C5) remains unauthorized. |
