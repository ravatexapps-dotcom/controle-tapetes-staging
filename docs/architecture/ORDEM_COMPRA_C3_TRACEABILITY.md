...
NEXT_AUTHORIZABLE_ACTION: a fresh Claude Code session performs PHASE-C5 / OC-C5-EMISSION-001 local UI implementation (docs/architecture/ORDEM_COMPRA_C5_PHASE_CONTRACT.md, STATUS: ACCEPTED / IMPLEMENTATION AUTHORIZED LOCALLY, §22) — wire the existing disabled oc-emitir button to native emitir_ordem_compra via the server-derived acoes.emitir signal, add the ratified CONTROLLED_IRREVERSIBLE_TRANSITION confirmation modal, and surface status_aceite, per the closed purely-additive three-file manifest (§12: ordem-compra-data.js/-render.js/-events.js only), entry gates (§15) and hard stops (§16); PHASE-C5B-ACCEPTANCE-DECISION (IDENTIFIED / NOT AUTHORIZED), any shared-database apply, staging validation/application of db/76 or db/77, deployment, activation, production access, and the REAL_CUTOVER window (OC-CUTOVER-001/OC-CUTOVER-PONR-001, hard-gated behind the mandatory separate read-only completeness disposition of the 13 unmapped ordens_compra_fio rows ids 153–165) remain unauthorized; no push
