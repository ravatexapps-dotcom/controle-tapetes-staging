...
NEXT_AUTHORIZABLE_ACTION: a fresh Claude Code session performs read-only diagnosis and documentation-only material-contract authoring of PHASE-C5A-DB-EMISSION-READINESS (the database-grant prerequisite for emitir_ordem_compra and alocar_necessidade_compra_fio, docs/architecture/ORDEM_COMPRA_C5_PHASE_CONTRACT.md §21/§5) — not issued or executed by this closeout; PHASE-C5 implementation, PHASE-C5B-ACCEPTANCE-DECISION (IDENTIFIED / NOT AUTHORIZED), and the REAL_CUTOVER window (OC-CUTOVER-001/OC-CUTOVER-PONR-001, hard-gated behind the mandatory separate read-only completeness disposition of the 13 unmapped ordens_compra_fio rows ids 153–165) remain unauthorized
