# AGENT_HANDOFF — Archived Operational Handoff Stack (2026-07)

> **Historical / non-operational archive.** This file preserves, verbatim and
> in its original chronological order (most-recent-first, exactly as it stood),
> the accumulated operational-handoff stack that lived in the root
> `AGENT_HANDOFF.md` before `GOVERNANCE-STATE-HANDOFF-COMPACTION-R1`
> (2026-07-20) replaced that file with a concise derived handoff.
>
> **Source document:** root `AGENT_HANDOFF.md` at commit
> `17ff8adddaa9f2fd3bc61af7261d9ebaad275f08` (the compaction baseline). No line
> below was rewritten; the content is the prior file byte-for-byte after this
> banner.
>
> **This archive carries NO live authority.** It is not a source of current
> state and holds no live next-action authority. Every "NEXT_AUTHORIZABLE_ACTION",
> "next authorizable action", status label, and boundary statement in the
> preserved blocks below is a **historical record of what was true when that block
> was written** and is superseded.
>
> - **Current operational state (sole owner):** `PROJECT_STATE.md`.
> - **Current operational handoff (derived, sole active handoff):** root
>   `AGENT_HANDOFF.md`.
> - **Append-only closed-phase history:** `docs/ledgers/G28_LEDGER.md`.
> - **Documentation authority / classification:** `docs/DOCUMENTATION_INDEX.md` §1.
>
> **Accepted vs. superseded:** the acceptance facts recorded below (accepted
> commits, staging migration identifiers, test counts, architect dispositions)
> remain accepted historical evidence, cross-recorded in
> `docs/ledgers/G28_LEDGER.md`. The forward-looking "next action" and "active
> phase" statements are superseded by `PROJECT_STATE.md`. In any divergence over
> a decision's wording, the ledger and `PROJECT_STATE.md` prevail over this
> archive. Append-only: do not edit or add new closeouts here.

---

## Preserved verbatim — root AGENT_HANDOFF.md as of 17ff8ad (superseded 2026-07-20)
...
- **`PHASE-C2` — `CLOSED / ACCEPTED`
  ...
  - Verified lock order: native order; items ascending; allocations ascending; idempotent
  ...
  - Read verification uses `obter_historico_recebimento_ordem_compra`, available only
  ...
  - Staging evidence passed: 48/48 focused tests; receipt/reversal/read-model/ACL/
