...
| `docs/architecture/PEDIDO_OP_SCHEMA_CONTRACT.md` | Technical contract | Schema contract for Pedido ↔ OP ↔ Movimentação ↔ Documentos. §13 is the accepted exact structural realization of the F1 executable contract: need-first RPC, immutable command journal, absolute-target mutation/cleanup, `(item_id, necessidade_id)` identity, derived quantity guard, ACL matrix, shared NULL-OP Phase C shape, errors, and lock order. §13.13 records F3R1; §13.14 records PHASE-C3A; §13.15 is the unchanged accepted C3B executable contract; §13.16 records C3C-A as `CLOSED / TECHNICALLY ACCEPTED — LOCALLY VERIFIED / INACTIVE / NOT APPLIED TO STAGING`, without granting staging, deployment, activation, cutover, or product acceptance. It records `READ-ONLY GOVERNANCE AND SPEC-CUSTODY AUDIT` as the next authorizable action while C3C-B remains unauthorized; operational state remains owned by `PROJECT_STATE.md`. |
| `docs/architecture/ORDEM_COMPRA_LIFECYCLE_SPEC_PROPOSED.md` | Ratified architectural contract | Governing four-layer purchase-order model. §R.27 records the accepted hybrid-origin addendum; §R.28 is the accepted exact F1 executable domain/API contract. §R.28.12 records F3R1; §R.28.13 records C3A; §R.29 remains the unchanged accepted C3B Phase-C3 executable contract, including the later single-window cutover, database-owned fence, deterministic import/reconciliation, ACL closure, and PONR recovery. §R.30 records C3C-A local technical acceptance and the `d4dba671` → `4b7ee13f` → `29913e40` → `89123729` chain, with no staging/application/deployment/activation/cutover/product acceptance. Its recorded next authorizable action is `READ-ONLY GOVERNANCE AND SPEC-CUSTODY AUDIT`; C3C-B remains unauthorized. C3D, staging application/validation, activation, deployment, real snapshot/import, fence transition, read switch, final ACL-closure invocation, cutover, C4, C5, production, `main`, remotes, and push also remain unauthorized. Acceptance history is append-only in `docs/ledgers/G28_LEDGER.md`; current state is in `PROJECT_STATE.md`. |
