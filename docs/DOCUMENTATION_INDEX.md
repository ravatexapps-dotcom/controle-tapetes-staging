...
| `docs/architecture/ORDEM_COMPRA_C5_PHASE_CONTRACT.md` | Material phase contract (implemented locally / targeted correction implemented / awaiting supervisor re-review; active) | Binds `OC-C5-EMISSION-001` to an exact functional scope (wire the existing disabled `oc-emitir` button to native `emitir_ordem_compra` + a confirmation modal + `status_aceite` display; no new list/history), an actor/state/action matrix keyed to the server `pode_emitir`/`bloqueio_emissao`/`acoes.emitir` model, an API ownership matrix (native `emitir_ordem_compra` only, excluding the superseded legacy flat `emitir_ordem_compra_fio`), a closed purely-additive three-file manifest (`ordem-compra-data.js`/`-render.js`/`-events.js`, no new product file), an idempotency/error contract, and a visual contract ratified `CONTROLLED_IRREVERSIBLE_TRANSITION` (§21). Created by `C4-CLOSEOUT-AND-C5-CONTRACT-R1` (2026-07-21); supervisor-accepted under `C5-CONTRACT-ACCEPTANCE-CLOSEOUT-R1` (§21) with the database prerequisite assigned to `PHASE-C5A-DB-EMISSION-READINESS`. That prerequisite is now closed (row below), so the supervisor **authorized local implementation** under `C5A-CLOSEOUT-AND-C5-AUTHORIZATION-R1` (2026-07-22, §22); local implementation then landed under `C5-PURCHASE-ORDER-EMISSION-UI-IMPLEMENTATION-R1` (§23) and, after direct supervisor functional/visual review found exactly one blocking functional defect (`C5_AMBIGUOUS_EMISSION_RELOAD_FALSE_DRAFT_ASSERTION`; visual review passed with nonblocking cosmetic debt), was targeted-corrected under `C5-AMBIGUOUS-RELOAD-AND-CANONICAL-STATE-CORRECTION-R1` (2026-07-22, §24): `STATUS: ACCEPTED / IMPLEMENTED LOCALLY / TARGETED CORRECTION IMPLEMENTED / AWAITING SUPERVISOR RE-REVIEW`. `OC-C5-EMISSION-001` is `PARTIALLY_SATISFIED` — not `SATISFIED`; the phase is not self-accepted and not closed. `PHASE-C5B-ACCEPTANCE-DECISION` remains `IDENTIFIED / NOT AUTHORIZED`. `PROJECT_STATE.md` remains sole owner of `ACTIVE_PHASE`/`ACTIVE_PHASE_CONTRACT` (now `PHASE-C5` / this file). |
