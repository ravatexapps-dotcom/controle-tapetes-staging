...
| `docs/architecture/ORDEM_COMPRA_C3D_PHASE_CONTRACT.md` | Material phase contract (proposed / awaiting supervisor acceptance / implementation not authorized; not active) | Binds the four already-ratified `OC-C3D-*` requirements (`OC-C3D-DEPLOY-001`/`OC-C3D-FENCE-001`/`OC-C3D-ACL-001`/`OC-C3D-LOCK-001`) to an isolated-rehearsal scope for `PHASE-C3D` (inactive deployment & rehearsal): six proposed sublots (C3D-A…C3D-F), environment strategy (disposable local PostgreSQL + read-only shared-DB inspection recommended; an isolated Supabase branch is UNPROVEN and not created), entry/exit gates, test matrix, recovery/PONR model, exact future manifests, and mandatory supervisor decisions. Creates no requirement, changes no anchor, and authorizes no implementation or environment action; every `OC-C3D-*` disposition is unchanged. Authored by `docs: accept C3C-B and define C3D contract` (docs-only + read-only diagnosis, 2026-07-21); two successive read-only supervisor reviews returned `CHANGES_REQUIRED` and the contract was forward-corrected twice the same day — by `docs: correct C3D contract boundaries` (§0/R1: requirement disposition, actor-path vs. structural eight-table fence proof, disposable-cluster-only PONR semantics, exact future manifest) and by `docs: finalize C3D contract execution boundaries` (§0b/R2: common documentary manifest for each C3D-A…E sublot, C3D-C reclassified as a database-faithful authenticated actor-context SQL proof, wildcard-wording correction). `PROJECT_STATE.md` remains sole owner of `ACTIVE_PHASE`/`ACTIVE_PHASE_CONTRACT` (both `NONE`). |
