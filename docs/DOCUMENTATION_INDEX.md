...
| `docs/architecture/PEDIDO_OP_SCHEMA_CONTRACT.md` | Technical contract | Schema contract for Pedido ↔ OP ↔ Movimentação ↔ Documentos. §13 is the exact structural realization of the F1 executable contract: need-first RPC, immutable command journal, absolute-target mutation/cleanup, `(item_id, necessidade_id)` identity, derived quantity guard, ACL matrix, shared NULL-OP Phase C shape, errors, and lock order. Awaiting architect acceptance; no implementation authorized. |
| `docs/architecture/ORDEM_COMPRA_LIFECYCLE_SPEC_PROPOSED.md` | Ratified architectural contract | Governing four-layer purchase-order model. §R.27 records the accepted hybrid-origin addendum; §R.28 closes the exact F1 executable domain/API contract and is awaiting architect acceptance. It defines OP-origin cotton, Pedido-origin shared polyester with NULL allocation OP, need-first absolute-target distribution, immutable actor/key idempotency, deterministic cleanup, allocation-derived item quantity, ACL disposition, Phase C shared shape, errors, and lock order. Acceptance history is append-only in `docs/ledgers/G28_LEDGER.md`; current state is in `PROJECT_STATE.md`. No implementation is authorized by the contract closure. |
