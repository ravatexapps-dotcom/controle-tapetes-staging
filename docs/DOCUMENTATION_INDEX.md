...
| `docs/architecture/PEDIDO_OP_SCHEMA_CONTRACT.md` | Technical contract | Schema contract for Pedido ↔ OP ↔ Movimentação ↔ Documentos. §13 is the accepted exact structural realization of the F1 executable contract: need-first RPC, immutable command journal, absolute-target mutation/cleanup, `(item_id, necessidade_id)` identity, derived quantity guard, ACL matrix, shared NULL-OP Phase C shape, errors, and lock order. §13.13 records the F3R1 acceptance disposition; §13.14 records PHASE-C3A technical acceptance; §13.15 is the accepted C3B executable contract: normalized reads, snapshot/import/reconciliation, state/fence/lock, ACL, and recovery. C3C inactive implementation requires a separate order; environment application is not authorized by the contract. |
| `docs/architecture/ORDEM_COMPRA_LIFECYCLE_SPEC_PROPOSED.md` | Ratified architectural contract | Governing four-layer purchase-order model. §R.27 records the accepted hybrid-origin addendum; §R.28 is the accepted exact F1 executable domain/API contract. §R.28.12 records F3R1; §R.28.13 records C3A technical acceptance; §R.29 is the accepted C3B Phase-C3 executable contract, including the later single-window cutover, database-owned fence, deterministic import/reconciliation, ACL closure, and recovery point of no return. Acceptance history is append-only in `docs/ledgers/G28_LEDGER.md`; current state is in `PROJECT_STATE.md`. C3C inactive implementation requires a separate order. |
