...
| `docs/architecture/PEDIDO_OP_SCHEMA_CONTRACT.md` | Technical contract | Schema contract for Pedido ↔ OP ↔ Movimentação ↔ Documentos. §13 is the accepted exact structural realization of the F1 executable contract. §13.15 remains the unchanged C3B executable contract; §13.16 records C3C-A local technical acceptance; §13.17 adds governance metadata for `OC-C3D-ACL-001` without changing product semantics. Current routing is owned by `PROJECT_STATE.md`; C3C-B remains unauthorized. |
| `docs/architecture/ORDEM_COMPRA_LIFECYCLE_SPEC_PROPOSED.md` | Ratified architectural contract | Governing four-layer purchase-order model. §R.29 remains the unchanged accepted Phase-C3 product contract; §R.30 records C3C-A local technical acceptance; §R.31 adds stable active-continuation requirement labels pointing to existing normative clauses without changing their semantics. Current routing is owned by `PROJECT_STATE.md`; C3C-B remains unauthorized. |
...
| `docs/architecture/ORDEM_COMPRA_C3C_B_PHASE_CONTRACT.md` | Material phase contract (accepted with blocking database prerequisites) | Binds `OC-C3-READ-001`/`OC-C3-WRITE-001`/`OC-C3-COMPAT-001`/`OC-C3-NOUI-001` to an exact repository scope, dependency inventory, implementation-file manifest, tests, evidence, and hard stops for a future `PHASE-C3C-B` application compatibility/adaptation implementation. Created by `C3C-B-MATERIAL-PHASE-CONTRACT-R1` (docs-only); a supervisor review returned `CHANGES_REQUIRED`, and the resulting forward correction was accepted (`STATUS: ACCEPTED_WITH_BLOCKING_DATABASE_PREREQUISITES / IMPLEMENTATION NOT AUTHORIZED`, §31 of that file). Does not authorize `PHASE-C3C-B`, which is additionally blocked pending `ORDEM_COMPRA_C3C_B_DB_PREREQUISITES_PHASE_CONTRACT.md` below. `PROJECT_STATE.md` remains sole owner of `ACTIVE_PHASE`/`ACTIVE_PHASE_CONTRACT` (both `NONE`). |
| `docs/architecture/ORDEM_COMPRA_C3C_B_DB_PREREQUISITES_PHASE_CONTRACT.md` | Material phase contract (accepted — nonblocking documentary debt; not active) | Defines two database components (canonical order-catalog projection; atomic legacy receipt-intent adapter) required to close the two hard stops recorded in `ORDEM_COMPRA_C3C_B_PHASE_CONTRACT.md` §25/§26, to migration-ready precision, for a future `PHASE-C3C-B-DB-PREREQ` implementation. Created by `C3C-B-DB-COMPATIBILITY-PREREQUISITES-CONTRACT-R1` (docs-only); R2-corrected, then its architecture accepted by the ratification closeout (`STATUS: ACCEPTED_WITH_NONBLOCKING_DOCUMENTARY_DEBT / IMPLEMENTATION NOT YET AUTHORIZED`, §34 of that file — both components inert-until-`canonical_active`; `db/76` = two functions + one additive `CHECK`, no bridge/backfill; fixed corpus binding; completeness/freeze/re-baseline deferred to real-cutover/C3D). Does not authorize implementation or any migration; `§R.29.7`/`§13.18` remain proposed, their normative application a prerequisite. `PROJECT_STATE.md` remains sole owner of `ACTIVE_PHASE`/`ACTIVE_PHASE_CONTRACT` (both `NONE`). |
