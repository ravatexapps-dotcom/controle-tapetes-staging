...
| `docs/architecture/CLEAN_SLATE_TRANSACTIONAL_RESET_PHASE_CONTRACT.md` | Material phase contract (proposed / awaiting supervisor review / destructive execution not authorized; not active) | Proposed contract for a controlled clean-slate rebuild of the operational transaction corpus in the non-production shared development database `ucrjtfswnfdlxwtmxnoo`, replacing the superseded legacy-data preservation/mapping strategy. Records the binding business-owner ruling (`CLEAN_SLATE_OPERATIONAL_REBUILD`), a complete read-only FK/dependency inventory and exact row-count baseline of the operational transaction domain, a per-table classification (`PRESERVE_MASTER_DATA` / `PURGE_OPERATIONAL_SOURCE` / `PURGE_OPERATIONAL_DERIVED` / `RESET_CUTOVER_METADATA` / `UNPROVEN`), the exact dependency-safe deletion order (Boundary A = self-contained yarn-purchasing corpus; Boundary B = `pedidos`/`ops`/`lotes`, only if separately authorized), a mandatory out-of-repo archival evidence plan, the destructive-execution design (single transaction, `DELETE` not `TRUNCATE`, run only under `legacy_active`, no default sequence reset, one-time governed administrative operation — not a `db/NN` migration, not the dashboard), the recommended Option C cutover strategy, the 13-row-gate consequence (`STILL_APPLICABLE_UNTIL_RESET_EXECUTION_COMPLETES` then `SUPERSEDED_BY_CLEAN_SLATE_RESET`), the `PHASE-C5B` sequencing, hard stops, validation/evidence matrices, and the supervisor decisions still required (incl. the binding Controlled-Delete × document-history entanglement for 1 Pedido + 4 OPs). Authored by `CLEAN-SLATE-TRANSACTIONAL-RESET-CONTRACT-R1` (2026-07-22, docs-only, no database mutation). Introduces no requirement ID and changes no disposition; authorizes no deletion, migration, cutover, activation, or environment change. `PROJECT_STATE.md` remains sole owner of `ACTIVE_PHASE`/`ACTIVE_PHASE_CONTRACT` (both `NONE`). |
