...
| `docs/architecture/ORDEM_COMPRA_LIFECYCLE_SPEC_PROPOSED.md` | Ratified architectural contract | Governing four-layer purchase-order model. Part R and §R.27 record the accepted hybrid-origin addendum: OP-origin cotton, Pedido-origin shared polyester with NULL allocation OP, NULL-safe shared identity, allocation-derived item quantity, Pedido/Insumos purchasing ownership, and Phase C forward-correction qualification. The accepted impact audit and addendum are referenced here without fabricating a standalone report; acceptance history is append-only in `docs/ledgers/G28_LEDGER.md`, current state in `PROJECT_STATE.md`. No implementation is authorized by the correction. |
