...
| `docs/governance/GOVERNANCE_EFFICIENCY_REFOUNDATION_PHASE_CONTRACT.md` | Active material phase contract for governance-efficiency refoundation; Units 1–3 are closed and directly accepted, the Unit 4 contract is corrected and awaiting direct supervisor review, structured sources remain non-canonical, current canonical owners are unchanged, and Unit 4A/documentary-authority cutover are not authorized. |
| `docs/governance/GOVERNANCE_EFFICIENCY_REFOUNDATION_UNIT_4_AUTHORITY_CUTOVER_CONTRACT.md` | Authored normative Unit 4 contract defining the future structured-authority architecture, 31-consumer migration matrix, schema transition, readiness gates, PONR, cutover boundaries, post-cutover validation, and forward-correction rollback; corrected after direct review to remove self-referential activation-commit identity, bind the required parent and deterministic activation manifest, confine Unit 4A renders to non-canonical candidate paths, and reserve atomic root replacement for Unit 4C. Awaiting direct supervisor review; Unit 4A implementation and authority cutover remain unauthorized. |
