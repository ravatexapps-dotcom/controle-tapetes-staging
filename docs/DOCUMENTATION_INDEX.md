...
| `docs/architecture/ORDEM_COMPRA_C3C_B_DB_PREREQUISITES_PHASE_CONTRACT.md` | Material phase contract (proposed) | Defines two database components (canonical order-catalog projection; atomic legacy receipt-intent adapter) required to close the two hard stops recorded in `ORDEM_COMPRA_C3C_B_PHASE_CONTRACT.md` §25/§26, to migration-ready precision, for a future `PHASE-C3C-B-DB-PREREQ` implementation. Created by `C3C-B-DB-COMPATIBILITY-PREREQUISITES-CONTRACT-R1` (docs-only). `STATUS: PROPOSED / AWAITING SUPERVISOR ACCEPTANCE / IMPLEMENTATION NOT AUTHORIZED`; does not authorize implementation or any migration. `PROJECT_STATE.md` remains sole owner of `ACTIVE_PHASE`/`ACTIVE_PHASE_CONTRACT` (both `NONE`). |
