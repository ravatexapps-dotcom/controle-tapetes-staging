...
| `docs/architecture/PEDIDO_OP_SCHEMA_CONTRACT.md` | Technical contract | Schema contract for Pedido ↔ OP ↔ Movimentação ↔ Documentos. §13 is the accepted exact structural realization of the F1 executable contract: need-first RPC, immutable command journal, absolute-target mutation/cleanup, `(item_id, necessidade_id)` identity, derived quantity guard, ACL matrix, shared NULL-OP Phase C shape, errors, and lock order. Accepted at `00897f09267fc8304b329ce46ba985d03a57faff`; §13.13 records the F3R1 acceptance disposition (scoped committed-concurrency-fixture waiver). Forward implementation is separately authorized, while environment application is not. |
| `docs/architecture/ORDEM_COMPRA_LIFECYCLE_SPEC_PROPOSED.md` | Ratified architectural contract | Governing four-layer purchase-order model. §R.27 records the accepted hybrid-origin addendum; §R.28 is the accepted exact F1 executable domain/API contract. It defines OP-origin cotton, Pedido-origin shared polyester with NULL allocation OP, need-first absolute-target distribution, immutable actor/key idempotency, deterministic cleanup, allocation-derived item quantity, ACL disposition, Phase C shared shape, errors, and lock order. §R.28.12 records the F3R1 acceptance disposition (scoped committed-concurrency-fixture waiver). Acceptance history is append-only in `docs/ledgers/G28_LEDGER.md`; current state is in `PROJECT_STATE.md`. Forward implementation is separately authorized; environment application is not. |
