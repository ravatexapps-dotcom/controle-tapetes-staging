...
- **Fase ativa:** nenhuma fase de implementação ativa. G28-C está fechado; G28-D permanece `NOT AUTHORIZED`.
...
- **Próxima ação autorizável:** nenhuma. `G28-D: NOT AUTHORIZED`; sem push; produção proibida. Débito não bloqueante: `AUTHENTICATED_BROWSER_SMOKE_BLOCKED_BY_TOOLING`.
...
- **Fases planejadas, não iniciadas (deferred):** G28-D (publicação) e posteriores — `NOT AUTHORIZED`. Não inferir autorização pela numeração.
