...
- **Frente ativa:** `G28-B7 — exibição nas superfícies — IMPLEMENTED / TESTED (local) / READY FOR ARCHITECT ACCEPTANCE`. Commit parcial Pedido-detail B7: `ed35f049397af4061ed6e8bb2d9ec3056c543724`. Todas as superfícies B7 implementadas: detalhe do Pedido (`DOCUMENTOS VINCULADOS` + `LINHA DO TEMPO DOS VINCULOS`), detalhe da OP (`op-latex-admin` + `op-tecelagem-producao-admin`: `Documentos vinculados` confirmed + timeline), timeline canônica de Pedido/OP (`buildDocumentLinkTimelineForPedido`/`buildDocumentLinkTimelineForOp`), busca global canônica na fila central (`confirmedPedidoId`/`confirmedOpId`/`linkAvailability` + `getConfirmedOpOptions`), helper UI compartilhado `js/document-links-surface-ui.js` (`buildLinkedDocumentNodes`/`buildLinkTimelineNodes`), reusando o read model e nunca lendo `pedido_manual`/`candidate.pedido_id` como vínculo. OP nunca inferida via filiação ao Pedido; estados indisponíveis fail-closed. Esta continuação same-phase completou as superfícies restantes após `ed35f04`; IAexec não cria fase nova nem auto-fecha. Resolve o HEAD final deste commit com `git rev-parse HEAD`.
- **Verificação remota exigida (não executada — Supabase proibido para o Claude):** render autenticado admin das novas seções contra staging `ucrjtfswnfdlxwtmxnoo`. Nenhuma query remota nova foi introduzida.
- **Testes locais (LF):** document-links-surface 14/14, document-surface-links-read-model 14/14, queue-ui 69/69, pedido-detail-linked-documents 7/7, bateria documental B4–B7 (19 arquivos) 616/616, bateria OP + ingestor UI 670 pass / 15 falhas pré-existentes (stale-regex em `ops-list-screen`/`op-form-helpers`/`op-writes` que leem `index.html` com regex estrito sem `?v=`, idênticas no baseline `ed35f04`); `op-latex-admin.smoke.js` (arquivo modificado) 55/0. Débitos pré-existentes inalterados: `pedido-detail.smoke.js` 140 pass / 41 fail (CRLF), `documents-ingestor.test.js` 2 fail, `g14-c-bridge-smoke.test.js` 15 fail.
...
- **Próxima ação autorizável:** aceite arquitetural de `G28-B7` (todas as superfícies B7 implementadas e testadas localmente). `G28-B8` não autorizado; sem push.
