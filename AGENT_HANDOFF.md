...
- **Última fase aceita:** `G28-CAMADA-2 / A4.2 — Guarda de Troca de Senha Obrigatória — CLOSED / ACCEPTED` (2026-07-16; ver seção própria abaixo e `PROJECT_STATE.md`). `CAMADA2-LAST-ACCESS-UI` (consumo da RPC `db/59` na UI, commit `0aff22f`) também `CLOSED / ACCEPTED` (2026-07-16 — validação visual do arquiteto confirmada em preview: coluna populada com dados reais, formato correto, `"—"` nos nunca-logados, ordenação com nulos por último). **Nenhum débito de closeout documental pendente entre essas duas fases.** `CLIENTE-ORDER-SUMMARY-READMODEL-ACL-GRANTS-R1` permanece `CLOSED / ACCEPTED` (2026-07-15). `G28-C` permanece a última fase funcional do G28 propriamente dito, agora `CLOSED / TECHNICALLY ACCEPTED — ARCHITECT PRODUCT VALIDATION PENDING` (reclassificação `G28-RECONCILIATION-DECISIONS-A`, 2026-07-15; matriz staging/projeções 16/16 PASS técnico; closeout histórico `a7d7caa`/aceite `d5ec09f` não reescrito; débito explícito `AUTHENTICATED_BROWSER_SMOKE_NOT_EXECUTED`). G28-B8 está `TECHNICALLY COMPLETED / ACCEPTANCE SUBSUMED BY G28-C`.
